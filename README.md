...
In the texts of Civizard, several systems of characters can be used: Hiragana, Katakana, Kanji, letters of the western/Latin alphabet, and western numerals. Studying the suspected font image, we can determine the font file contains Hiragana, Katakana, western letters in the expected order, only some Kanja, and special symbols such as cursor arrows; most Kanji and all western numerals are missing. The font's characters have the right size, so it is most probably the right font. If this is indeed the font image, the game most likely picks the characters from the file and puts them into texts such as the one below.  
According to google lens, the Kanji there seem to describe units, buildings and the like. Since we are missing the Kanji, maybe this is the file for the Kanji (but unlikely, since some characters are repeated and the text is pretty narrow). Unfortunately the machine translation only gives a very rough translation and neither of us is fluent in Japanese, so we have to wait till we can dive a deeper into this and also get confirmation from a Japanese speaker. We will know more after decompressing all the bmf files so we can have a look for a file with the Kanji.   
...
+ Technical information on the tim format https://web.archive.org/web/20230702165945/https://rsync.irixnet.org/tutorials/pstutorials/chapter1/3-textures.html
