...
Explanation:
??? It turned out the correct version of Python script is missing... the one linked in this github repo cannot unpack the terrain tiles correctly. The script seems to be an older version.
??? His comment on this after sending the decompressed files: "when i did the initial unpacking, my code wasn't smart enough to understand to decompress those files so this is a quick 'n dirty hackjob i did — that zip contains the world0 and world1 folders but with the appropriate files decompressed" 
...
this is being worked on nothing to see here yet ...
_________
### 5. Results
