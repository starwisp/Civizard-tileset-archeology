...
_________
??? delete later
Combining the suspected palette files with the graphics data files after they were generated by the Python script, showed something akin to terrain tiles but the result was still not correct.???

![tiledggd-pe- tiles directly after script](https://github.com/starwisp/Civizard-tileset-archeology/assets/4465384/ec475d2a-5770-4e9a-8948-1fa04f4a4380) 
<b>decompressed terrain graphics data united with palette data ??? most probably needs to be removed</b>  

Explanation:
??? It turned out the correct version of Python script is missing... the one linked in this github repo cannot unpack the terrain tiles correctly. Though he sent me the mostly correct files, called *.bin.out in an zip, the script seems to be an older version.
??? Raccoonsam'S comment on this after sending the decompressed files: "when i did the initial unpacking, my code wasn't smart enough to understand to decompress those files so this is a quick 'n dirty hackjob i did — that zip contains the world0 and world1 folders but with the appropriate files decompressed" 
??? is it possible to get the correct script, to enhance the one in this repo or to get a description of what he did to make this work? 
_________
### 5. Results
...
![Pixels shifted 8 pixel columns horizontally 1 pixel vertically](https://github.com/starwisp/Civizard-tileset-archeology/assets/4465384/d4d8f61e-177c-4999-9a26-2ef330684b58)
...
+ Video on theoretical background of the decompressor script using a similar case as example (Tomba 2 for PSX) https://www.youtube.com/watch?v=LASE8IiGB9Q
