...
The answer, we suspect may lie in an interesting animation technique. Many of these tiles look muted compared to their in-game appearance. This is due to some clever CLUT animations. The palettes if these terrain tiles get changed fast enough that it appears as an animation - with different tiles being swapped in as animation phases. But instead of different tiles as animation phases (such as with monster tiles), it is the same tile but in different palettes. This leads to for example the shore wave animations or glow effects in Civizard. This technique is called canvas cycling. See an example for that in further information ('Example of canvas cycling').  
