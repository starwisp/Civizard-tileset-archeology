...
Speculation: Studying the image, we can determine the font file contains hiragana, katakana, arabic letters and numbers; most Kanji are still missing. As far as we can tell, the game possibly picks the characters from the font file above and puts them into texts such as the one below. Another possiblity is that the file below is also part of the font but for certain Kanji since according to google lens, Kanji there seem to describe units, buildings and the like. Unfortunately the machine translation only gives a very rough translation and neither of us is fluent in Japanese, so we have to wait till we can dive a deeper into this and also get confirmation from a Japanese speaker. We also will probably not get around decompressing all the bmf files in order to find the full font including the Kanji.  
